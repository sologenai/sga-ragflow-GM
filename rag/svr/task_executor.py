...
async def do_handle_task(task):
    ...
    # Either using graphrag or Standard chunking methods
        ...
        try:
            ...
            ok, kb = KnowledgebaseService.get_by_id(task_dataset_id)
            if not ok:
                progress_callback(prog=-1.0, msg="Cannot find valid knowledgebase for GraphRAG task")
                return

            kb_parser_config = kb.parser_config
            
            # If no GraphRAG configuration exists, create default configuration
            if not kb_parser_config.get("graphrag", {}).get("use_graphrag", False):
                progress_callback(prog=0.15, msg="Initializing default GraphRAG configuration...")
                kb_parser_config.update(
                    {
                        "graphrag": {
                            "use_graphrag": True,
                ...
                if not KnowledgebaseService.update_by_id(kb.id, {"parser_config": kb_parser_config}):
                    progress_callback(prog=-1.0, msg="Internal error: Failed to update GraphRAG configuration")
                    return
                progress_callback(prog=0.2, msg="GraphRAG configuration initialized successfully")
            ...
            async with kg_limiter:
                ...
                # Log detailed result
                if result:
                    logging.info(f"GraphRAG task completed for kb {task_dataset_id}:\n{result}")
                    progress_callback(prog=1.0, msg=f"Knowledge Graph done successfully ({timer() - start_ts:.2f}s)")
                else:
                    logging.warning(f"GraphRAG task returned no result for kb {task_dataset_id}")
                    progress_callback(prog=-1.0, msg=f"Knowledge Graph processing failed")
