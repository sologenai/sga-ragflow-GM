...
### 5.1 侧边栏图谱菜单改为轻量存在性检查

发现数据集左侧栏为了判断是否显示“知识图谱”菜单，会在每个数据集子页面调用 `GET /kb/<id>/knowledge_graph`。

图谱生成前该接口基本为空，响应很轻；图谱生成后该接口会进入大图谱统计/预览逻辑，导致文件列表、测试、日志、配置等页面切换都被图谱接口拖慢。

本次新增：

```text
GET /kb/<id>/knowledge_graph?exists_only=1
```

侧边栏只调用 `exists_only=1`，后端只判断是否存在图谱数据，不再加载预览节点、边、统计。只有真正进入“知识图谱”页面时才加载图谱预览。
