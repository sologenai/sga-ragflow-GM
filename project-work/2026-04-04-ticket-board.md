...
| TK-013 | GraphRAG large-scale embedding throughput and fault-tolerance remediation | `agent-B` | `dispatched` | `high` | Special remediation for private embedding timeout on 32k-node GraphRAG jobs: batch embedding, dedicated concurrency, retries, queueing, configurable timeout, and large-graph fault tolerance |
