...
| TK-012 | GraphRAG checkpoint resume completion for timeout/quota failures | `agent-B` | `accepted` | `high` | Document-level checkpointing is now wired into execution, partial failures keep the task resumable, resume skips durable merged docs, and SDK/UI paths share resume semantics |
| TK-013 | GraphRAG large-scale embedding throughput and fault-tolerance remediation | `agent-B` | `accepted` | `high` | Accepted after agent-x validation: GraphRAG node/edge embedding now uses bounded batching, dedicated concurrency, retry/backoff, configurable timeout, and resumable failure context |
| TK-014 | Mainstream embedding vector-dimension compatibility remediation | `agent-vector` | `accepted` | `high` | Accepted after agent-x rework: ES/OpenSearch cover mainstream dimensions, existing-index repair is idempotent, and runtime dimensions under the ES 4096 limit are handled without another code change |
...
- `TK-012` was dispatched by `agent-x` to `agent-B` as the completion pass for GraphRAG checkpoint resume after timeout/quota failures. It closes the gap left by TK-010 by wiring doc-level monitor state into the real execution path and making partial failures resumable instead of silently successful.
- `TK-013` was dispatched by `agent-x` to `agent-B` as a GraphRAG large-scale embedding throughput and fault-tolerance remediation ticket. It is triggered by a remote private embedding timeout near `32709/32713` nodes and must address batching, backpressure, retries, configurable timeout, and resumable failure handling.
- `TK-013` is now accepted after agent-x validation and minor acceptance rework. The final implementation batches node/edge embedding, uses a dedicated GraphRAG embedding limiter, retries transient embedding failures, keeps old graph deletion after vector preparation, and exposes task timeout controls.
- `TK-014` was dispatched by `agent-x` to `agent-vector` as a standalone vector-index compatibility remediation ticket. It is triggered by remote Qwen embedding configuration failures suspected to come from unsupported vector dimensions and must cover mainstream dimensions plus safe repair of existing indexes.
- `TK-014` is now accepted after agent-x validation and one small acceptance rework. The final behavior keeps mainstream dynamic templates, repairs existing ES/OpenSearch indexes non-destructively, accepts runtime ES dimensions up to 4096 even when not in the mainstream list, and reports dimensions above the ES backend limit as actionable errors.
