...
const GraphPerformanceOptimizer: React.FC<GraphPerformanceOptimizerProps> = ({
  ...
  const nodeImportanceMap = useMemo(() => {
    ...
    nodes.forEach((node) => {
      ...
      const normalizedDegree =
        degree / Math.max(1, Math.max(...Array.from(degreeMap.values())));
  ...
  const visibleNodes = useMemo(() => {
    ...
    const priorityNodes = filteredAndSortedNodes.filter(
      (node) => node.id === selectedNodeId || node.id === highlightedNodeId,
    );
  ...
  const getNodeImportance = useCallback(
