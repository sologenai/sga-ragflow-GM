...
The `ragflow_retrieval` tool also accepts `use_kg`. Set `use_kg` to `true` to include knowledge graph retrieval evidence when the target dataset already has a generated GraphRAG graph. This parameter only affects retrieval; it does not generate a graph automatically.
